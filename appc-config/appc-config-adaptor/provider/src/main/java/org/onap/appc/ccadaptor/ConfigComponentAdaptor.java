...
public class ConfigComponentAdaptor implements SvcLogicAdaptor {
    ...
    public ConfigStatus configure(String key, Map<String, String> parameters, SvcLogicContext ctx) {
        ...
        if (areEqual(chg, action, ACTION_PREPARE)) {
            return prepare(ctx, "CHANGE", OPERATION_CHANGE);
        }
        if (areEqual(chg, action, ACTION_ACTIVATE)) {
        ...
        if (areEqual(scale, action, ACTION_PREPARE)) {
            return prepare(ctx, "CHANGE", OPERATION_SCALE);
        }
        if (areEqual(scale, action, ACTION_ACTIVATE)) {
            return activate(ctx, true);
        }
    ...
    private boolean areEqual(String chg, String action, String acn) {
        return chg != null && action != null && action.equalsIgnoreCase(acn);
    }
