...
public class ParseAdminArtifcat implements SvcLogicJavaPlugin {
    ...
    protected static Map<String, String> parseAdminArtifact(ArtifactMapper js) {
        ...
        Consumer<FqdnList> fqdnConsumer = new Consumer<FqdnList>() {
            public void accept(FqdnList listFqdn) {
                Consumer<CloudOwnerList> clList = new Consumer<CloudOwnerList>() {
                    @Override
                    public void accept(CloudOwnerList clList) {
                        Consumer<RegionIdList> rgList = new Consumer<RegionIdList>() {
                            @Override
                            public void accept(RegionIdList rgList) {

                                Consumer<String> tenantList = new Consumer<String>() {

                                    @Override
                                    public void accept(String tenantId) {
                                        tenantId.toString();
                                        mp.put(clList.getCloudOwner().trim() + ";" + rgList.getRegionId().trim() + ";"
                                                + tenantId, listFqdn.getVnfManagementServerFqdn());
                                        log.info("list of  mapped details  from admin artifact"
                                                + clList.getCloudOwner().trim() + ";" + rgList.getRegionId().trim()
                                                + ";" + tenantId + "-->" + listFqdn.getVnfManagementServerFqdn());
                                    }

                                };
                                rgList.getTenantIdList().forEach(tenantList);
                                ;
                                // TODO Auto-generated method stub
                                rgList.getTenantIdList();
                                rgList.getRegionId();
                            }

                        };
                        // TODO Auto-generated method stub
                        clList.getRegionIdList().forEach(rgList);
                        ;
                    }
                };
                listFqdn.getCloudOwnerList().forEach(clList);
                // TODO Auto-generated method stub
            }

        };
        fqdnList.forEach(fqdnConsumer);
        return mp;
    ...
    public String retrieveFqdn(SvcLogicContext ctx)
        ...
        log.info("tenantid--cloudowner--cloudregion retrieved from payload :" + ctx.getAttribute("payloadTenant")+":"+ctx.getAttribute("payloadCloudOwner")+":"+ctx.getAttribute("payloadCloudRegion"));
        log.info("tenantid--cloudowner--cloudregion retrieved from a&ai : " + ctx.getAttribute("tenantAai")+":"+ctx.getAttribute("cloudOwneraai")+":"+ctx.getAttribute("cloudRegionAai"));
        try {
            ...
            Consumer<String> fqdnMap = new Consumer<String>() {
                @Override
                public void accept(String t) {
                    if (t.contains(key) || t.equals(key)) {
    ...
    public String getAdminArtifact(SvcLogicContext ctx) {
        ...
        try {
            String query = "SELECT ARTIFACT_CONTENT as adminjson" + " FROM ASDC_ARTIFACTS "
                    + "WHERE ARTIFACT_NAME = '" + artifcatName + "' " + "ORDER BY INTERNAL_VERSION DESC LIMIT 1 ";
