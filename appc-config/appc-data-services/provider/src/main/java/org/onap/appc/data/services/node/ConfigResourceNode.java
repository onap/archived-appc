...
public class ConfigResourceNode implements SvcLogicJavaPlugin {
    ...
    private static final String FILE_CATEGORY = "file-category";
    private static final String TMP_PREPARELEL = "tmp.preparerel";
    private static final String UNABLE_TO_SAVE = "Unable to save prepare_relationship";
    private static final String TMP_CONVERT_CONFIG = "tmp.convertconfig.escapeData";
    private static final String CONFIG_PARAMS = "configuration-params";
    private static final String TMP_MERGED_DATA = "tmp.merge.mergedData";
    ...
    public void getConfigFileReference(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            if (status == QueryStatus.NOT_FOUND || status == QueryStatus.FAILURE) {
                throw new QueryException(UNABLE_TO_READ + "ConfigFileReference:device-configuration");
            }
            ...
            if (status == QueryStatus.NOT_FOUND || status == QueryStatus.FAILURE) {
                throw new QueryException(UNABLE_TO_READ + "ConfigFileReference:configuration_success");
            }
            ...
            if (status == QueryStatus.NOT_FOUND || status == QueryStatus.FAILURE) {
                throw new QueryException(UNABLE_TO_READ + "ConfigFileReference:configuration_error");
            }

            status = db.getConfigFileReferenceByFileTypeNVnfType(ctx, "configfilereference-log", "log");

            if (status == QueryStatus.NOT_FOUND || status == QueryStatus.FAILURE) {
                throw new QueryException(UNABLE_TO_READ + "ConfigFileReference:configuration_log");
            }
            ...
            log.error("An error occurred in GetConfigFileReference", e);
    ...
    public void getCommonConfigInfo(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            if (status == QueryStatus.NOT_FOUND || status == QueryStatus.FAILURE) {
                throw new QueryException(UNABLE_TO_READ + "device_interface_protocol");
            }

            status = db.getConfigureActionDGByVnfTypeNAction(ctx, "tmp.configureactiondg");
            if (status == QueryStatus.FAILURE) {
                throw new QueryException(UNABLE_TO_READ + "configure_action_dg");
            }
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in getCommonConfigInfo", e);
    ...
    public void getTemplate(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            if (StringUtils.isBlank(templateName)) {
                ...
                if (status == QueryStatus.FAILURE) {
                    throw new QueryException(UNABLE_TO_READ + fileCategory);
                }
                ...
                if (status == QueryStatus.NOT_FOUND || status == QueryStatus.FAILURE) {
                    throw new QueryException(UNABLE_TO_READ + fileCategory + " template");
                }
            }
            ...
            log.error("An error occurred in getTemplate", e);

            throw new SvcLogicException(e.getMessage());
        }
    }

    private void saveConfigFiles(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            if (status == QueryStatus.FAILURE) {
                throw new QueryException("Unable to Save " + ctx.getAttribute(FILE_CATEGORY) + " in configfiles");
            }

            status = db.getMaxConfigFileId(ctx, "tmp.configfilesmax", ctx.getAttribute(FILE_CATEGORY));

            if (status == QueryStatus.NOT_FOUND || status == QueryStatus.FAILURE) {
                throw new QueryException("Unable to get " + ctx.getAttribute(FILE_CATEGORY) + " from configfiles");
            }

            status = db.savePrepareRelationship(ctx, TMP_PREPARELEL,
                ctx.getAttribute("tmp.configfilesmax.configfileid"), "N");
            if (status == QueryStatus.FAILURE) {
                throw new QueryException(UNABLE_TO_SAVE);
            }
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in saveConfigFiles", e);
    ...
    public void updateUploadConfig(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in updateUploadConfig", e);
    ...
    public void savePrepareRelationship(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            QueryStatus status = db.savePrepareRelationship(ctx, TMP_PREPARELEL, fileId, sdcArtifactInd);
            if (status == QueryStatus.FAILURE) {
                throw new QueryException(UNABLE_TO_SAVE);
            }
    ...
    public void saveConfigBlock(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            responsePrefix = StringUtils.isNotBlank(responsePrefix) ? (responsePrefix + ".") : "";
            ctx.setAttribute(TMP_CONVERT_CONFIG, EscapeUtils.escapeSql(ctx.getAttribute("configuration")));

            if (StringUtils.isBlank(ctx.getAttribute(CONFIG_PARAMS))) {
                saveDeviceConfiguration(inParams, ctx, "Request", ctx.getAttribute(TMP_CONVERT_CONFIG),
                    ctx.getAttribute("configuration"));
            } else {

                saveConfigurationBlock(inParams, ctx);

                ctx.setAttribute(TMP_CONVERT_CONFIG,
                    EscapeUtils.escapeSql(ctx.getAttribute(TMP_MERGED_DATA)));
                saveDeviceConfiguration(inParams, ctx, "Configurator", ctx.getAttribute(TMP_CONVERT_CONFIG),
                    ctx.getAttribute(TMP_MERGED_DATA));
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in saveConfigBlock", e);
    ...
    public void saveTemplateConfig(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            if (StringUtils.isBlank(ctx.getAttribute(CONFIG_PARAMS))) {
                ...
                saveDeviceConfiguration(inParams, ctx, "Template", ctx.getAttribute(TMP_CONVERT_CONFIG),
            ...
            QueryStatus status = db.savePrepareRelationship(ctx, TMP_PREPARELEL,
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in saveTemplateConfig", e);
    ...
    public void saveStyleSheetConfig(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            ctx.setAttribute(TMP_CONVERT_CONFIG,
                EscapeUtils.escapeSql(ctx.getAttribute(TMP_MERGED_DATA)));
            saveDeviceConfiguration(inParams, ctx, "StyleSheet", ctx.getAttribute(TMP_CONVERT_CONFIG),
                ctx.getAttribute(TMP_MERGED_DATA));
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in saveStyleSheet", e);
    ...
    private void saveDeviceConfiguration(Map<String, String> inParams, SvcLogicContext ctx, String dataSource,
        ...
        ctx.setAttribute(FILE_CATEGORY, "device_configuration");
    ...
    private void saveConfigurationBlock(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        ctx.setAttribute(FILE_CONTENT, ctx.getAttribute(TMP_CONVERT_CONFIG));
        ctx.setAttribute(FILE_CATEGORY, "configuration_block");
        saveConfigFiles(inParams, ctx);
    }
    ...
    private void saveConfigurationData(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        ctx.setAttribute(FILE_CATEGORY, "config_data");
        saveConfigFiles(inParams, ctx);
    }
    ...
    public void getConfigFilesByVnfVmNCategory(Map<String, String> inParams, SvcLogicContext ctx)
        ...
        try {
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in getConfigFilesByVnfVmNCategory", e);
    ...
    public void getDownloadConfigTemplateByVnf(Map<String, String> inParams, SvcLogicContext ctx)
        ...
        try {
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in getDownloadConfigTemplateByVnf", e);
    ...
    public void saveConfigTransactionLog(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in saveConfigTranseactionLog", e);
            throw new SvcLogicException(e.getMessage());
        }
    }
    ...
    public void getVnfcReference(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        try {
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in getVnfcReference", e);
    ...
    public void getCapability(Map<String, String> inParams, SvcLogicContext ctx) throws SvcLogicException {
        ...
        if (!checkIfCapabilityCheckNeeded(caplevel, findCapability)) {
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_SUCCESS);
            log.info("getCapability Successful - No need for capability templateChecker for this action");
            return;
        }
        try {
            DGGeneralDBService db = DGGeneralDBService.initialise();
            ...
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_STATUS,
                AppcDataServiceConstant.OUTPUT_STATUS_FAILURE);
            ctx.setAttribute(responsePrefix + AppcDataServiceConstant.OUTPUT_PARAM_ERROR_MESSAGE, e.getMessage());
            log.error("An error occurred in getCapability", e);
    ...
    public void processCapabilitiesForVMLevel(String vServerId, SvcLogicContext ctx, String findCapability,
        ...
        JsonNode vmCaps = getJsonNode(findCapability, subCapabilities);
        ...
        String capability = vmCaps.toString().contains(vnfcFunctionCode) ? SUPPORTED : NOT_SUPPORTED;
        ctx.setAttribute(CAPABILITIES, capability);

        log.info("End processCapabilitiesForVMLevel():capabilities is ::" + ctx.getAttribute(CAPABILITIES));
    }

    private JsonNode getJsonNode(String findCapability, JsonNode subCapabilities) {
