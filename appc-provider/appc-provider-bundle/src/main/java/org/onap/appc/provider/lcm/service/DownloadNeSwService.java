...
/**
 * Provide LCM command service for Downloading Ne Sw
 */
public class DownloadNeSwService extends AbstractBaseService {
	/**
     * Constructor
     */
    public DownloadNeSwService() {
        super(Action.DownloadNeSw);
        logger.debug("DownloadNeSwService starts");
    }
    ...
    void validate(CommonHeader commonHeader, Action action, ActionIdentifiers actionIdentifiers, Payload payload) {
        ...
        try {
            ...
            logger.error(String.format("DownloadNeSwService (%s) got IOException when converting payload", rpcName), e);
