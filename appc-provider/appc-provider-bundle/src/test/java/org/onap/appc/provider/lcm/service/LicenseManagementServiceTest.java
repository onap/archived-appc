...
public class LicenseManagementServiceTest {
    private final Action myAction = Action.LicenseManagement;
    private final String PAYLOAD_STRING = "{\"A\":\"A-value\",\"B\":{\"C\":\"B.C-value\",\"D\":\"B.D-value\"}}";
    private LicenseManagementInput mockInput = mock(LicenseManagementInput.class);
    ...
    private LicenseManagementService licenseManagementAction;
    @Before
    public void setUp() throws Exception {
        licenseManagementAction = spy(new LicenseManagementService());
    }

    @Test
    public void testProcess() throws Exception {
        // test error occurs in validation
        LicenseManagementOutputBuilder outputBuilder = licenseManagementAction.process(mockInput);
        Mockito.verify(licenseManagementAction, times(0)).proceedAction(any(),any(),any());
      ...
      //  whenNew(RequestExecutor.class).withNoArguments().thenReturn(mockExecutor);
        ...
        try {
            outputBuilder = licenseManagementAction.process(mockInput);
        }catch(Exception e){
    ...
    public void testValidate() throws Exception {
        licenseManagementAction.validate(mockCommonHeader, Action.LicenseManagement, mockAI,mockPayload);
        Status status = (Status) Whitebox.getInternalState(licenseManagementAction, "status");
        ...
        Mockito.verify(licenseManagementAction, times(0)).buildStatusForParamName(any(), any());
        Mockito.verify(licenseManagementAction, times(0)).buildStatusForErrorMsg(any(), any());
        ...
        // test empty action
        licenseManagementAction.validate(mockCommonHeader, Action.LicenseManagement, mockAI,mockPayload);
        status = (Status) Whitebox.getInternalState(licenseManagementAction, "status");
        ...
        // test null payload
        Mockito.doReturn("vnfId").when(mockAI).getVnfId();
        licenseManagementAction.validate(mockCommonHeader, Action.LicenseManagement, mockAI, null);
        Mockito.verify(licenseManagementAction, times(1)).validateExcludedActIds(any(), any());
        status = (Status) Whitebox.getInternalState(licenseManagementAction, "status");
