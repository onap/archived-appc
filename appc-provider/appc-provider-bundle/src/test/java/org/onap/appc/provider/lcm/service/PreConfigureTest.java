...
public class PreConfigureTest {
    private final Action myAction = Action.PreConfigure;
    private final String PAYLOAD_STRING = "{\"A\":\"A-value\",\"B\":{\"C\":\"B.C-value\",\"D\":\"B.D-value\"}}";
    private PreConfigureInput mockInput = mock(PreConfigureInput.class);
    ...
    private PreConfigure preConfigureAction;
    @Before
    public void setUp() throws Exception {
        preConfigureAction = spy(new PreConfigure());
    }

    @Test
    public void testProcess() throws Exception {
        // test error occurs in validation
        PreConfigureOutputBuilder outputBuilder = preConfigureAction.process(mockInput);
        Mockito.verify(preConfigureAction, times(0)).proceedAction(any(),any(),any());
      ...
      //  whenNew(RequestExecutor.class).withNoArguments().thenReturn(mockExecutor);
        ...
        try {
            outputBuilder = preConfigureAction.process(mockInput);
        }catch(Exception e){
    ...
    public void testValidate() throws Exception {
        preConfigureAction.validate(mockCommonHeader, Action.PreConfigure, mockAI,mockPayload);
        Status status = (Status) Whitebox.getInternalState(preConfigureAction, "status");
        ...
        Mockito.verify(preConfigureAction, times(0)).buildStatusForParamName(any(), any());
        Mockito.verify(preConfigureAction, times(0)).buildStatusForErrorMsg(any(), any());
        ...
        // test empty action
        preConfigureAction.validate(mockCommonHeader, Action.PreConfigure, mockAI,mockPayload);
        status = (Status) Whitebox.getInternalState(preConfigureAction, "status");
        ...
        // test null payload
        Mockito.doReturn("vnfId").when(mockAI).getVnfId();
        preConfigureAction.validate(mockCommonHeader, Action.PreConfigure, mockAI, null);
        Mockito.verify(preConfigureAction, times(1)).validateExcludedActIds(any(), any());
        status = (Status) Whitebox.getInternalState(preConfigureAction, "status");
