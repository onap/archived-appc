...
public class PreRebuildTest {
    private final Action myAction = Action.PreRebuild;
    private final String PAYLOAD_STRING = "{\"A\":\"A-value\",\"B\":{\"C\":\"B.C-value\",\"D\":\"B.D-value\"}}";
    private PreRebuildInput mockInput = mock(PreRebuildInput.class);
    ...
    private PreRebuild preRebuildAction;
    @Before
    public void setUp() throws Exception {
        preRebuildAction = spy(new PreRebuild());
    }

    @Test
    public void testProcess() throws Exception {
        // test error occurs in validation
        PreRebuildOutputBuilder outputBuilder = preRebuildAction.process(mockInput);
        Mockito.verify(preRebuildAction, times(0)).proceedAction(any(),any(),any());
      ...
      //  whenNew(RequestExecutor.class).withNoArguments().thenReturn(mockExecutor);
        ...
        try {
            outputBuilder = preRebuildAction.process(mockInput);
        }catch(Exception e){
    ...
    public void testValidate() throws Exception {
        preRebuildAction.validate(mockCommonHeader, Action.PreRebuild, mockAI,mockPayload);
        Status status = (Status) Whitebox.getInternalState(preRebuildAction, "status");
        ...
        Mockito.verify(preRebuildAction, times(0)).buildStatusForParamName(any(), any());
        Mockito.verify(preRebuildAction, times(0)).buildStatusForErrorMsg(any(), any());
        ...
        // test empty action
        preRebuildAction.validate(mockCommonHeader, Action.PreRebuild, mockAI,mockPayload);
        status = (Status) Whitebox.getInternalState(preRebuildAction, "status");
        ...
        // test null payload
        Mockito.doReturn("vnfId").when(mockAI).getVnfId();
        preRebuildAction.validate(mockCommonHeader, Action.PreRebuild, mockAI, null);
        Mockito.verify(preRebuildAction, times(1)).validateExcludedActIds(any(), any());
        status = (Status) Whitebox.getInternalState(preRebuildAction, "status");
