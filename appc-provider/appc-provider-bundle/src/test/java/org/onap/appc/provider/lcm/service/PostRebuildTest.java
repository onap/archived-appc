...
public class PostRebuildTest {
    private final Action myAction = Action.PostRebuild;
    private final String PAYLOAD_STRING = "{\"A\":\"A-value\",\"B\":{\"C\":\"B.C-value\",\"D\":\"B.D-value\"}}";
    private PostRebuildInput mockInput = mock(PostRebuildInput.class);
    ...
    private PostRebuild postRebuildAction;
    @Before
    public void setUp() throws Exception {
        postRebuildAction = spy(new PostRebuild());
    }

    @Test
    public void testProcess() throws Exception {
        // test error occurs in validation
        PostRebuildOutputBuilder outputBuilder = postRebuildAction.process(mockInput);
        Mockito.verify(postRebuildAction, times(0)).proceedAction(any(),any(),any());
      ...
      //  whenNew(RequestExecutor.class).withNoArguments().thenReturn(mockExecutor);
        ...
        try {
            outputBuilder = postRebuildAction.process(mockInput);
        }catch(Exception e){
    ...
    public void testValidate() throws Exception {
        postRebuildAction.validate(mockCommonHeader, Action.PostRebuild, mockAI,mockPayload);
        Status status = (Status) Whitebox.getInternalState(postRebuildAction, "status");
        ...
        Mockito.verify(postRebuildAction, times(0)).buildStatusForParamName(any(), any());
        Mockito.verify(postRebuildAction, times(0)).buildStatusForErrorMsg(any(), any());
        ...
        // test empty action
        postRebuildAction.validate(mockCommonHeader, Action.PostRebuild, mockAI,mockPayload);
        status = (Status) Whitebox.getInternalState(postRebuildAction, "status");
        ...
        // test null payload
        Mockito.doReturn("vnfId").when(mockAI).getVnfId();
        postRebuildAction.validate(mockCommonHeader, Action.PostRebuild, mockAI, null);
        Mockito.verify(postRebuildAction, times(1)).validateExcludedActIds(any(), any());
        status = (Status) Whitebox.getInternalState(postRebuildAction, "status");
