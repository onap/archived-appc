...
public class ProvisioningServiceTest {
    private final Action myAction = Action.Provisioning;
    private final String PAYLOAD_STRING = "{\"A\":\"A-value\",\"B\":{\"C\":\"B.C-value\",\"D\":\"B.D-value\"}}";
    private ProvisioningInput mockInput = mock(ProvisioningInput.class);
    ...
    private ProvisioningService provisioningAction;
    @Before
    public void setUp() throws Exception {
        provisioningAction = spy(new ProvisioningService());
    }

    @Test
    public void testProcess() throws Exception {
        // test error occurs in validation
        ProvisioningOutputBuilder outputBuilder = provisioningAction.process(mockInput);
        Mockito.verify(provisioningAction, times(0)).proceedAction(any(),any(),any());
      ...
      //  whenNew(RequestExecutor.class).withNoArguments().thenReturn(mockExecutor);
        ...
        try {
            outputBuilder = provisioningAction.process(mockInput);
        }catch(Exception e){
    ...
    public void testValidate() throws Exception {
        provisioningAction.validate(mockCommonHeader, Action.Provisioning, mockAI,mockPayload);
        Status status = (Status) Whitebox.getInternalState(provisioningAction, "status");
        ...
        Mockito.verify(provisioningAction, times(0)).buildStatusForParamName(any(), any());
        Mockito.verify(provisioningAction, times(0)).buildStatusForErrorMsg(any(), any());
        ...
        // test empty action
        provisioningAction.validate(mockCommonHeader, Action.Provisioning, mockAI,mockPayload);
        status = (Status) Whitebox.getInternalState(provisioningAction, "status");
        ...
        // test null payload
        Mockito.doReturn("vnfId").when(mockAI).getVnfId();
        provisioningAction.validate(mockCommonHeader, Action.Provisioning, mockAI, null);
        Mockito.verify(provisioningAction, times(1)).validateExcludedActIds(any(), any());
        status = (Status) Whitebox.getInternalState(provisioningAction, "status");
