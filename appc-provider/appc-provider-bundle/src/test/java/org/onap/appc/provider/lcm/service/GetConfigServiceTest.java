...
public class GetConfigServiceTest {
    private final Action myAction = Action.GetConfig;
    private final String PAYLOAD_STRING = "{\"A\":\"A-value\",\"B\":{\"C\":\"B.C-value\",\"D\":\"B.D-value\"}}";
    private GetConfigInput mockInput = mock(GetConfigInput.class);
    ...
    private GetConfigService getConfigureAction;
    @Before
    public void setUp() throws Exception {
        getConfigureAction = spy(new GetConfigService());
    }

    @Test
    public void testProcess() throws Exception {
        // test error occurs in validation
        GetConfigOutputBuilder outputBuilder = getConfigureAction.process(mockInput);
        Mockito.verify(getConfigureAction, times(0)).proceedAction(any(),any(),any());
      ...
      //  whenNew(RequestExecutor.class).withNoArguments().thenReturn(mockExecutor);
        ...
        try {
            outputBuilder = getConfigureAction.process(mockInput);
        }catch(Exception e){
    ...
    public void testValidate() throws Exception {
        getConfigureAction.validate(mockCommonHeader, Action.GetConfig, mockAI,mockPayload);
        Status status = (Status) Whitebox.getInternalState(getConfigureAction, "status");
        ...
        Mockito.verify(getConfigureAction, times(0)).buildStatusForParamName(any(), any());
        Mockito.verify(getConfigureAction, times(0)).buildStatusForErrorMsg(any(), any());
        ...
        // test empty action
        getConfigureAction.validate(mockCommonHeader, Action.GetConfig, mockAI,mockPayload);
        status = (Status) Whitebox.getInternalState(getConfigureAction, "status");
        ...
        // test null payload
        Mockito.doReturn("vnfId").when(mockAI).getVnfId();
        getConfigureAction.validate(mockCommonHeader, Action.GetConfig, mockAI, null);
        Mockito.verify(getConfigureAction, times(1)).validateExcludedActIds(any(), any());
        status = (Status) Whitebox.getInternalState(getConfigureAction, "status");
