...
public class ConfigArtifactProcessor extends AbstractArtifactProcessor {
    ...
    private final EELFLogger logger =
            EELFManager.getInstance().getLogger(ConfigArtifactProcessor.class);
     ...
     */
    public ConfigArtifactProcessor(IDistributionClient client, EventSender eventSender,
            INotificationData notification, IResourceInstance resource, IArtifactInfo artifact,
            URI storeUri) {
    ...
    public void processArtifact(SDCArtifact artifact) throws APPCException {
        String postData = Util.toSdcStoreDocumentInput(notification, resource, super.artifact,
                artifact.getArtifactContent());
        logger.debug("ConfigArtifactProcessor::processArtifact::postData=" + postData);
        try {
            if (null != storeUri) {
                logger.debug(
                        "ConfigArtifactProcessor::processArtifact::URI is" + storeUri.toString());
                ProviderResponse result = ProviderOperations.post(storeUri.toURL(), postData, null);
                if (result.getStatus() == 200) {
                    logger.debug(
                            "ConfigArtifactProcessor::processArtifact::post request success!!");
                    Util.parseResponse(result.getBody());
                } else {
                    logger.debug(
                            "ConfigArtifactProcessor::processArtifact()::post request failed!! Returned :"
