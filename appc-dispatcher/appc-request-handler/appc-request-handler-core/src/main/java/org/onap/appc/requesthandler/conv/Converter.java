...
public class Converter {
    ...
    public static Flags convFlagsMapTorev160108Flags(org.onap.appc.domainmodel.lcm.Flags flags) {
        ...
        if (flags.containsKey(FORCE_FLAG)) {
            Flags.Force force = Flags.Force.valueOf(flags.get(FORCE_FLAG).toString());
            flagsBuilder.setForce(force);
            anyFlag = true;
        }
        if (flags.containsKey(MODE_FLAG)) {
            Flags.Mode mode = Flags.Mode.valueOf(flags.get(MODE_FLAG).toString());
