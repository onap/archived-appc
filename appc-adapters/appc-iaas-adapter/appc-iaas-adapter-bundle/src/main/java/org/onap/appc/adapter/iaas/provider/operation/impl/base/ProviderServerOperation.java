...
public abstract class ProviderServerOperation extends ProviderOperation {
    ...
    /**
     * Looks up the indicated server using the provided context and returns the
     ...
     *             If the server cannot be found because we cant connect to the
     *             provider
     */
    ...
    /**
     * Enter a pool-wait loop checking the server state to see if it has entered one
     * of the desired states or not.
     * <p>
     * This method checks the state of the server periodically for one of the
     * desired states. When the server enters one of the desired states, the method
     * returns a successful indication (true). If the server never enters one of the
     * desired states within the allocated timeout period, then the method returns a
     * failed response (false). No exceptions are thrown from this method.
     * </p>
     *
     * @param rc
     *            The request context that manages the state and recovery of the
    ...
    protected void waitForStateChange(RequestContext rc, Server server, Server.Status... desiredStates)
        ...
        while (rc.attempt()) {
            try {
                try {
                    if (server.getStatus().equals(Server.Status.ERROR)) {
                        ...
                        logger.info(msg);
                        msg = EELFResourceManager.format(Msg.MIGRATE_SERVER_FAILED, provider.getName(),
                                service.getURL());
                        logger.error(msg);
            ...
            } catch (ContextConnectionException ex) {
                List<String> list = new ArrayList<>();
                for (Server.Status desiredState : desiredStates) {
                    list.add(desiredState.name());
                }
                String reason = EELFResourceManager.format(Msg.CONNECTION_FAILED_RETRY, ex.getClass().getSimpleName(),
                        "server", server.getName(), server.getId(), StringHelper.asList(list),
                        server.getStatus().name(), ex.getMessage());
                logger.error(reason);
                logger.error(EELFResourceManager.format(ex));
                msg = "Device status" + Server.Status.ERROR + "cannot proceed further";
                if (server.getStatus().equals(Server.Status.ERROR)) {
                    throw new RequestFailedException("Migrate Server", msg, HttpStatus.CONFLICT_409, server);
                } else {
     ...
     * @param context
     *            The interface cloud service provider to access services or the
     *            object model, or both
     */
    protected void checkVirtualMachineNetworkStatus(RequestContext rc, Server server, Context context)
        ...
        for (Port port : ports) {
            ...
            case "PENDING":
                /*
                 * The port's status is changing because of some event or operation. The final
    ...
    private void validateNetwork(RequestContext rc, Server server, Port port, Network network)
        ...
        if (!network.getStatus().equals(Network.Status.ACTIVE.toString())) {
            msg = createErrorMessage(rc, server, port);
            throw new RequestFailedException("VM Server Network is DOWN", msg, HttpStatus.PRECONDITION_FAILED_412,
                    server);
        }
    }
