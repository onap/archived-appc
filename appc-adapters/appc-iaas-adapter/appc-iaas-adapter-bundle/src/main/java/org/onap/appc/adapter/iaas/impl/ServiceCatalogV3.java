...
/**
 * This class is used to capture and cache the service catalog for a specific OpenStack provider. <p> This is needed
 * because the way the servers are represented in the ECOMP product is as their fully qualified URL's. This is very
 * problematic, because we cant identify their region from the URL, URL's change, and we cant identify the versions of
 * the service implementations. In otherwords, the URL does not provide us enough information. </p> <p> The zone
 * abstraction layer is designed to detect the versions of the services dynamically, and step up or down to match those
 * reported versions. In order to do that, we need to know before hand what region we are accessing (since the supported
 * versions may be different by regions). We will need to authenticate to the identity service in order to do this, plus
 * we have to duplicate the code supporting proxies and trusted hosts that exists in the abstraction layer, but that
 * cant be helped. </p> <p> What we do to circumvent this is connect to the provider using the lowest supported identity
 * api, and read the entire service catalog into this object. Then, we parse the vm URL to extract the host and port and
 * match that to the compute services defined in the catalog. When we find a compute service that has the same host name
 * and port, whatever region that service is supporting is the region for that server. </p> <p> While we really only
 * need to do this for compute nodes, there is no telling what other situations may arise where the full service catalog
 * may be needed. Also, there is very little additional cost (additional RAM) associated with caching the full service
 * catalog since there is no way to list only a portion of it. </p>
 */
public class ServiceCatalogV3 extends ServiceCatalog {
    ...
    private void addRegions(Service service, String type) {
        ...
        for (Endpoint endpoint : endpoints) {
            serviceEndpoints.computeIfAbsent(type, val -> Lists.newArrayList(endpoint));
