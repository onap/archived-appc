...
public class RestHealthcheckActivatorTest {
    ...
    private RestHealthcheckActivator restHealthcheckActivator = new RestHealthcheckActivator();

    @Before
    public void setUp() {
        given(bundleContext.registerService(eq(RestHealthcheckAdapter.class), isA(RestHealthcheckAdapterImpl.class), isNull(
    ...
    public void start_shouldRegisterService_whenRegistrationOccursForTheFirstTime() throws Exception {
        ...
        then(bundleContext).should(only())
            .registerService(eq(RestHealthcheckAdapter.class), isA(RestHealthcheckAdapterImpl.class), isNull(
                Dictionary.class));
    }
    ...
    public void start_shouldRegisterServiceOnlyOnce_whenServiceRegistrationIsNotNull() throws Exception {
        ...
        // THEN
        then(bundleContext).should(only()).registerService(eq(RestHealthcheckAdapter.class), isA(RestHealthcheckAdapterImpl.class), isNull(
            Dictionary.class));
    }
    ...
    private void registerService() throws Exception {
        restHealthcheckActivator.start(bundleContext);
    }

    private void unregisterService() throws Exception {
        restHealthcheckActivator.stop(bundleContext);
    }
}
