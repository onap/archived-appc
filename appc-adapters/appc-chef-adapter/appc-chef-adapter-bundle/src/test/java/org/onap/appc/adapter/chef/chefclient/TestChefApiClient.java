...
public class TestChefApiClient {
    ...
    private void thenStringShouldMatch(String method, String[] response) throws ParseException {
        ...
        assertTrue(checkTimestamp(response[3], 5000));
        assertEquals("X-Ops-UserId:test", response[4]);
    }

    private boolean checkTimestamp(String timeStampHeader, long maxDeltaMs) {
        if (!timeStampHeader.startsWith("X-Ops-Timestamp:")) {
            return false;
        }
        LocalDateTime ld1 = LocalDateTime.parse(timeStampHeader.replace("X-Ops-Timestamp:", ""), dtf);
        return ChronoUnit.MILLIS.between(ld1, LocalDateTime.now(ZoneId.of("UTC"))) <= maxDeltaMs;
    }
}